...
int round_close(Round* r) {
    ...
    // -------------------------------------------------
    // 7) 清理每位玩家的局状态（释放 Pattern，但保留结算缓存字段）
    // -------------------------------------------------
    for (int i = 0; i < n; ++i) {
        PlayerRound* pr = r->players[i];
        if (!pr) continue;
        // PlayerRound::resetRound() 负责释放 Pattern、重置标记
        // 注意：结算缓存字段（m_settled*）不会被清空，
        //       下一节会写入本局结果
        pr->resetRound();
    }

    // -------------------------------------------------
    // 8) 将结算结果写入每个 PlayerRound 对象（C API 可读）
    //    必须在 resetRound 之后写入，这些字段不会被 resetRound 清空
    // -------------------------------------------------
    for (int i = 0; i < n; ++i) {
        PlayerRound* pr = r->players[i];
        if (!pr) continue;
        ...
        // 将调整后的净得分累加到累计总分
        pr->addScore(net_scores[i] - round_scores[i]);

        pr->setSettledNetScore(net_scores[i]);
        pr->setSettledFouled(fouled[i]);
        pr->setSettledHomerun(homerun[i]);

        // 计算打枪/被打枪次数
        int shoot_cnt = 0, shot_cnt = 0;
        for (int j = 0; j < n; ++j) {
            if (i == j || fouled[j]) continue;
            // pair_results 只存了 i<j 的半三角
            if (i < j) {
                PairSettleResult p = pair_results[i * n + j];
                if (p.winner != 0) {
                    if (p.shoot_a) shoot_cnt++;
                    if (p.shoot_b) shot_cnt++;
                }
            } else {
                PairSettleResult p_rev = pair_results[j * n + i];
                if (p_rev.winner != 0) {
                    if (p_rev.shoot_b) shoot_cnt++;
                    if (p_rev.shoot_a) shot_cnt++;
                }
            }
        }
        pr->setSettledShootCnt(shoot_cnt);
        pr->setSettledShotCnt(shot_cnt);
    }
...
/* ================================================================
 *  extern "C" 导出: 供 Python ctypes / Dart FFI 直接调用
 * ================================================================ */
