...
namespace evaluate
{
    ...
    bool submit(HandManager *mgr, Pattern *pat)
    {
        ...
        for (int i = 0; i < 3; ++i)
            pat->head[i] = mgr->m_hand[mgr->m_pile[0][i]];
        for (int i = 0; i < 5; ++i) {
            pat->middle[i] = mgr->m_hand[mgr->m_pile[1][i]];
            pat->tail[i] = mgr->m_hand[mgr->m_pile[2][i]];
        }
        return true;
    }
    ...
    {
        if (!mgr || idx < 0 || idx > 12) return -1;
        return static_cast<int>(mgr->m_status[idx]);
    }

    int getPileCount(HandManager *mgr, int position)
    {
        if (!mgr || position < 0 || position > 2) return -1;
        return mgr->m_pile_cnt[position];
    }

    int getPileCard(HandManager *mgr, int position, int index)
    {
        if (!mgr || position < 0 || position > 2) return -1;
        if (index < 0 || index >= mgr->m_pile_cnt[position]) return -1;
        return mgr->m_pile[position][index];
    }
}
