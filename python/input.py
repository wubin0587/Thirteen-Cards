...
# 添加 MinGW DLL 搜索路径（仅Windows）
if os.name == 'nt':
    mingw_dir = os.environ.get('MINGW_DLL_DIR', r"D:\Program Files (x86)\mingw64\bin")
    try:
        os.add_dll_directory(mingw_dir)
    except (OSError, AttributeError):
        pass  # 目录不存在或版本不支持时静默跳过
...
def _declare_signatures(lib: ctypes.CDLL) -> None:
    ...
    # 比较函数 (closer.cpp extern "C" 导出)
    lib.tc_compare_head_3cards.argtypes = [
        ctypes.POINTER(ctypes.c_int),  # cards_a[3]
        ctypes.POINTER(ctypes.c_int),  # cards_b[3]
    ]
    lib.tc_compare_head_3cards.restype = ctypes.c_int

    lib.tc_compare_five_cards.argtypes = [
        ctypes.POINTER(ctypes.c_int),  # cards_a[5]
        ctypes.POINTER(ctypes.c_int),  # cards_b[5]
    ]
    lib.tc_compare_five_cards.restype = ctypes.c_int

    lib.tc_compare_hand_result.argtypes = [ctypes.c_int, ctypes.c_int]
    lib.tc_compare_hand_result.restype = ctypes.c_int
...
def py_compare_head_3cards(cards_a: List[int], cards_b: List[int]) -> int:
    """
    调用 C++ tc_compare_head_3cards, 返回 1(a赢)/-1(b赢)/0(平).
    cards_a, cards_b 各恰好 3 张牌.
    """
    if len(cards_a) != 3 or len(cards_b) != 3:
        raise ValueError("head cards must be exactly 3")
    lib = get_lib()
    arr_a = (ctypes.c_int * 3)(*cards_a)
    arr_b = (ctypes.c_int * 3)(*cards_b)
    return lib.tc_compare_head_3cards(arr_a, arr_b)


def py_compare_five_cards(cards_a: List[int], cards_b: List[int]) -> int:
    """
    调用 C++ tc_compare_five_cards, 返回 1(a赢)/-1(b赢)/0(平).
    cards_a, cards_b 各恰好 5 张牌.
    """
    if len(cards_a) != 5 or len(cards_b) != 5:
        raise ValueError("five cards must be exactly 5")
    lib = get_lib()
    arr_a = (ctypes.c_int * 5)(*cards_a)
    arr_b = (ctypes.c_int * 5)(*cards_b)
    return lib.tc_compare_five_cards(arr_a, arr_b)


def py_compare_hand_result(rank_order_a: int, rank_order_b: int) -> int:
    """调用 C++ tc_compare_hand_result, 纯 rank_order 比较."""
    lib = get_lib()
    return lib.tc_compare_hand_result(rank_order_a, rank_order_b)
